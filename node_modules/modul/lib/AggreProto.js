...
function AggreProto(data){
	...
	this.$publish("create",Object.create(this._data));
}
...
AggreProto.prototype = {
	...
	$getAggreType : function(AggreTypeName){
	...
	$data:function(k,v){
		if(arguments.length === 0){
				...
				this._data[k] = v;
				this.constructor._db.save(this.constructor.typeName,this.data(k),function(){})
				this.$publish("change",this.data(k));
		}
	},

	remove:function(){
		this.constructor._cache[this.id] = false;
		this.constructor._db.remove(this.id);
		this.constructor._db.remove(this.constructor.typeName,this.id,function(){});
		this.$publish("remove",this.id);
	},
